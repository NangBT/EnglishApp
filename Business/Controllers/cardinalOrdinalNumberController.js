...
function validationNumber() {
  ...
  for (var i = 1; i < lengthOfRows; i++) {
    ...
    var isCardinalNumber = isValidCell(
      numberValue,
      row,
      "cardinalNumberCol",
      cardinalOrdinalNumberConstant.type.cardinal
    );
    var isOrdinalNumber = isValidCell(
      numberValue,
      row,
      "ordinalNumberCol",
      cardinalOrdinalNumberConstant.type.ordinal
    );
    var isAcronym = isValidCell(
      numberValue,
      row,
      "acronymCol",
      cardinalOrdinalNumberConstant.type.ordinal
...
function isValidCell(numberValue, row, classCell, type) {
  ...
  if (txt.val().length > 0) {
    ...
    if (type === cardinalOrdinalNumberConstant.type.cardinal) {
      numberText = cardinalOrdinalNumberService.convertNumberToText(parseInt(numberValue), type
      );
    } else {
      numberText = cardinalOrdinalNumberService.convertOrdinalNumberToText(parseInt(numberValue));
    }
    ...
    if (classCell === "acronymCol") {
      if (numberText !== "") {
        ...
        var acronymText = utilities.formatNumber(numberValue) + twoCharacterLast.toLowerCase();
